...
class GoveeMQTT:
            ...
            await self.client.publish(
                ...
                json.dumps(
                    await self.handle_command(
                        message.topic.value.removeprefix(self.prefix), payload
                    )
                ),
            )
    ...
    async def on_recv(self, cmd: int, key: int, data: bytes):
        match cmd:
            case govee.CMD_READ:
                ...
                await self.client.publish(
                    self.notify, json.dumps({"register": key, "data": data.hex()})
                )

            case govee.CMD_WRITE | govee.CMD_MULTI:
                await self.client.publish(
                    self.ack, json.dumps({"data": bytes([key, *data]).hex()})
                )
    ...
    async def on_send(self, cmd: int, key: int, data: bytes):
        ...
        await self.client.publish(
            self.send, json.dumps({"cmd": cmd, "register": key, "data": data.hex()})
        )

    async def on_error(self, *args):
        await self.client.publish(self.error, json.dumps({"data": args}))

    async def handle_command(self, cmd: str, data: str):
        if (m := CMD_RE.match(cmd)) is None:
        ...
        try:
            match cmd.lower():
                ...
                case "mode":
                    match await self.dev.get_mode():
                        ...
                        case govee.SceneMode(code, name):
                            mode = {"mode": "scene", "code": code, "name": name}
                ...
                case "version":
                    match num:
                        case "1":
                            version = await self.dev.get_version()
                        case "2":
                            version = await self.dev.get_hwver()
                        case "3":
                            version = await self.dev.get_fwver()
                        case _:
                            version = (
                ...
                case "brightness":
                    if data := data.strip().lower():
                        ...
                        await self.dev.set_brightness(brightness, int(num or -1))
                        return {"Brightness": brightness}
                    else:
                        if not num:
                            color = await self.dev.get_segments()
                            return {"Brightness": [c.brightness for c in color]}

                        if segment := int(num):
                            color = await self.dev.get_segments(segment)
                ...
                case "color":
                    if data := data.strip().lower():
                        if c := COLOR_RE.match(data):
                            ...
                            await self.dev.set_color(
                                cast(govee.RGB, color), int(num or -1)
                            )
...
async def main():
    async with govee.scan() as light:
        _ = light.keepalive()
        mqtt = GoveeMQTT(light, broker="theseus.home.arpa", topic="govee")
        async with mqtt:
            await mqtt.serve()
